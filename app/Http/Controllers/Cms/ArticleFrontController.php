    ...
    public function news_list(Request $request)
    {
        ...
        if(isset($_GET['type'])){
            ...
            if($_GET['type'] == 'searchbox'){
                ...
                $f_criteria = explode("-", $_GET['criteria']);                          
                $articles = Article::whereYear('date', $f_criteria[0])->whereMonth('date', $f_criteria[1])->whereStatus('Published');
            } else {
            ...
            $articles = $articles->orderBy('date', 'asc')->paginate($pageLimit);
                ...
                <li>
                    ...
                    foreach($months as $month){
                        ...
                        if(isset($_GET['type'])){
                            if($_GET['type'] == 'month'){
                                $f_criteria = explode("-", $_GET['criteria']);
                                if($year->yr == $f_criteria[0] && $month->mo == $f_criteria[1]){
                                    $current_ym = 'class="current"';
                                }
                            }
                        }


                        $data .= '<li '.$current_ym.'><a class="menu-link" href="'.route('news.front.index').'?type=month&criteria='.$year->yr.'-'.$month->mo.'"><div>'.date("F", mktime(0, 0, 0, $month->mo, 1)).' ('.$month->total_articles.')</div></a></li>';
                    }
